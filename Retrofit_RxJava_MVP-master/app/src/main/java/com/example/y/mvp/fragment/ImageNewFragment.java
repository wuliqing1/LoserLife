 ...
 */
public class ImageNewFragment extends BaseFragment<BaseView.ImageNewView, ImageNewPresenterImpl>
        implements BaseView.ImageNewView, SwipeRefreshLayout.OnRefreshListener, MyRecyclerView.LoadingData, BaseRecyclerViewAdapter.OnItemClickListener<ImageNewInfo> {
    ...
    @Override
    protected ImageNewPresenterImpl createPresenter() {
        return new ImageNewPresenterImpl();
    }
    ...
    public void onRefresh() {
        adapter.removeAll();
        mPresenter.requestNetWork(etId.getText().toString().trim(), etRows.getText().toString().trim());
    }
